    ...
    const VCC = parseFloat(urlParams.get('VCC'));
    const R1 = parseFloat(urlParams.get('R1'));
    const R2 = parseFloat(urlParams.get('R2'));
    const RE = parseFloat(urlParams.get('RE'));
    const VBE = parseFloat(urlParams.get('VBE'));
    const RC = parseFloat(urlParams.get('RC'));
    const RG = parseFloat(urlParams.get('RG'));
    const RY = parseFloat(urlParams.get('RY'));
    const Vin = parseFloat(urlParams.get('Vin'));
...
var VoutValues = VinValues.map(Vin => KVG*Vin);

                // AC Sonuçlarını textarea'ya yaz
                var 
                result_ac = "Giriş Direnci (Ri): " + RB_rn_paralel + "\n\n";
...
ctx.fillStyle = '#000';
const maxV = Math.max(...VinValues, ...VoutValues); // Maksimum gerilim değerini bul
const minV = Math.min(...VinValues, ...VoutValues); // Minimum gerilim değerini bul
const rangeV = maxV - minV; // Maksimum ve minimum arasındaki fark
const stepY = rangeV / 5; // Y ekseninde gösterilecek değer aralıkları
